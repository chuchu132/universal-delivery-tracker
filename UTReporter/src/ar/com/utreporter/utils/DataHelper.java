...
import ar.com.utreporter.Config;
...
public class DataHelper {
	...
	public JSONArray getMyTicket(Context ctx){
		TelephonyManager telephonyManager = (TelephonyManager)ctx.getSystemService(Context.TELEPHONY_SERVICE);
		ArrayList<NameValuePair> par = new ArrayList<NameValuePair>();
		par.add(new BasicNameValuePair("imei", telephonyManager.getDeviceId()));
		par.add(new BasicNameValuePair("user_id", "" + AppUserData.getInstance().user_id));
		String resp = HttpHelper.postData(Config.BASEURL + Config.TRACKER_CONTROLLER + "get_tickets_by_client_device", par);
		try {
			return new JSONArray(resp);
		} catch (JSONException e) {
			e.printStackTrace();
	...
	public void setTicketEntregado(Context ctx, String ticket_id){
		TelephonyManager telephonyManager = (TelephonyManager)ctx.getSystemService(Context.TELEPHONY_SERVICE);
		ArrayList<NameValuePair> par = new ArrayList<NameValuePair>();
		par.add(new BasicNameValuePair("imei", telephonyManager.getDeviceId()));
		par.add(new BasicNameValuePair("user_id", "" + AppUserData.getInstance().user_id));
		par.add(new BasicNameValuePair("ticket_id", ticket_id));
		HttpHelper.postData(Config.BASEURL + Config.TRACKER_CONTROLLER + "set_ticket_entregado", par);
	}
